...
class YDMFranchiseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "distributor",
            "new_order_count",
            "franchise_contact_numbers",
        ]

    def get_new_order_count(self, obj):
        return Order.objects.filter(
            logistics="YDM", franchise=obj, order_status="Sent to YDM"
        ).count()

    def get_franchise_contact_numbers(self, obj):
        users = CustomUser.objects.filter(franchise=obj, role="Franchise").values(
...
class UserSmallSerializer(serializers.ModelSerializer):
    ...
    def get_franchise_contact_numbers(self, obj):
        users = CustomUser.objects.filter(
            franchise=obj.franchise, role="Franchise"
