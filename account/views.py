...
class UserRetrieveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        # Update username with phone number
...
class YDMFranchiseListCreateView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Franchise.objects.annotate(
            order_count=Count(
                "orders",
                filter=Q(orders__logistics="YDM", orders__order_status="Sent to YDM"),
            )
...
class UserDistributorListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            distributors = Distributor.objects.filter(factory=request.user.factory)
...
class SalesPersonListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if user.role in ["Franchise", "Packaging"]:
            ...
            franchises = Franchise.objects.filter(distributor__factory=user.factory)
            return CustomUser.objects.filter(
                franchise__in=franchises,
                role="SalesPerson",
                is_active=True,
